...
class ContactUtilisateur(models.Model):
    utilisateur = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='contacts')
    type_contact = models.ForeignKey(TypeContact, on_delete=models.CASCADE)
    valeur = models.CharField(max_length=100)  # Ex: le numéro ou l'email

    def __str__(self):
        return f"{self.utilisateur} - {self.type_contact}: {self.contact}"
