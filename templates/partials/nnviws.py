...
def communaute(request):
    ...
    avis_par_prestataire = {
        p.id: p.avis_set.order_by('-date').first()
...
def reserver_service(request, service_id):
    ...
    if request.user.role != 'client':
        messages.error(request, "Seuls les clients peuvent réserver.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            conflit = Reservation.objects.filter(
                service_offert=service_offert, date=date, heure=heure,
            ...
            if conflit:
                ...
                notifier(
                    ...
                    f"Nouvelle demande pour le service {service.nom}."
                )
    ...
    return render(request, 'reservation/reserver_service.html', {
        ...
        'prestataire': prestataire, 'service_offert': service_offert
    })

@login_required
def mes_reservations(request):
    reservations = Reservation.objects.filter(client=request.user).order_by('-date')
...
def mes_reservations_prestataire(request):
    ...
    reservations = Reservation.objects.filter(service_offert__prestataire=prestataire).order_by('-date')
    for r in reservations:
        r.nb_messages_non_lus = Message.objects.filter(
            reservation=r, recipient=request.user, is_read=False
        ).count()
    return render(request, 'reservation/mes_reservations_prestataire.html', {'reservations': reservations})

# mes services et ajout/modification/suppression de services
@login_required
def mes_services(request):
    prestataire = Prestataire.objects.get(utilisateur=request.user)
    services_offerts = ServiceOffert.objects.filter(prestataire=prestataire)
    return render(request, 'reservation/mes_services.html', {'services': services_offerts})
...
def ajouter_service_offert(request):
    ...
    return render(request, 'reservation/ajouter_service.html', {
        'form': form, 'services_autorises': SERVICES_AUTORISES
    })
...
def modifier_service(request, service_id):
    ...
    return render(request, 'reservation/modifier_service.html', {
        'form': form, 'service': service_offert
    })
...
# laisser un avis et détails prestataire/prestataire
@login_required
def laisser_avis(request, prestataire_id):
    prestataire = get_object_or_404(Prestataire, id=prestataire_id)
    avis = Avis.objects.filter(client=request.user, prestataire=prestataire).first()
    is_update = avis is not None
...
def detail_prestataire(request, prestataire_id):
    ...
    reservations_terminees = Reservation.objects.filter(
        client=request.user, service_offert__prestataire=prestataire, statut='terminée'
    ).exists() if request.user.is_authenticated and request.user.role == 'client' else False
...
def completer_profil_prestataire(request):
    ...
    if Prestataire.objects.filter(utilisateur=request.user).exists():
        messages.info(request, "Profil prestataire déjà existant.")
        return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            prestataire.utilisateur = request.user
            prestataire.save()
            messages.success(request, "Profil prestataire créé.")
            return redirect('dashboard')
    else:
...
def signaler_prestataire(request, prestataire_id):
    ...
    if not Reservation.objects.filter(client=request.user, service_offert__prestataire=prestataire, statut='terminée').exists():
        messages.error(request, "Vous devez avoir réservé ce prestataire pour le signaler.")
        return redirect('mes_reservations')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=f"Vous avez été signalé pour : {signalement.motif}",
                auto=True
            )
            send_mail(
                "Signalement reçu sur Elfeservice",
...
def defense_signalement(request, signalement_id):
    ...
    if request.method == 'POST':
        signalement.defense = request.POST.get('defense')
        preuve = request.FILES.get('preuve_defense')
        if preuve:
            signalement.preuve_defense = preuve
        signalement.save()
        messages.success(request, "Défense enregistrée.")
        return redirect('dashboard')
...
def avertir_prestataire(request, signalement_id):
    ...
    Notification.objects.create(
        ...
        message=f"Avertissement reçu : {signalement.motif}",
        auto=True
    )
    send_mail(
        "Avertissement reçu",
        f"Vous avez été averti pour : {signalement.motif}.",
        "admin@elfeservice.com",
...
def bannir_prestataire(request, signalement_id):
    ...
    PrestataireBanni.objects.create(
        email=user.email,
        telephone=getattr(prestataire, 'telephone', ''),
        nom=user.name
    )
    ...
    Notification.objects.create(
        utilisateur=user,
        message=f"Votre compte a été banni pour : {signalement.motif}",
        auto=True
    )
    send_mail(
        "Compte banni",
        f"Votre compte a été banni pour : {signalement.motif}.",
        "admin@elfeservice.com",
        [user.email]
    )
    prestataire.delete()
